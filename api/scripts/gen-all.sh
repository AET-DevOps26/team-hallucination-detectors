...
#   services/scanner-service/src/generated/java  — Spring interfaces for the internal contract
...
GEN_DIR="$(mktemp -d)"
SCANNER_GEN_DIR="$(mktemp -d)"
trap 'rm -rf "$GEN_DIR" "$SCANNER_GEN_DIR"' EXIT
...
echo "==> Generating Spring server interfaces (scanner-service)"
npx --yes @openapitools/openapi-generator-cli@2.15.3 generate \
  --openapitools "$ROOT/api/openapitools.json" \
  -i "$ROOT/api/scanner-internal.yaml" \
  -g spring \
  -o "$SCANNER_GEN_DIR" \
  --api-package de.tum.devops.vibeshield.scanner.generated.api \
  --model-package de.tum.devops.vibeshield.scanner.generated.model \
  --global-property "apis=Scanner,models,apiDocs=false,modelDocs=false,apiTests=false,modelTests=false" \
  --additional-properties "interfaceOnly=true,useSpringBoot3=true,useTags=true,openApiNullable=false,skipDefaultInterface=true,hideGenerationTimestamp=true,useBeanValidation=true"

SCANNER_TARGET="$ROOT/services/scanner-service/src/generated/java"
rm -rf "$SCANNER_TARGET"
