...
describe("AnalysisListPage", () => {
  ...
  it("navigates to analysis detail on card click", () => {
    const nav = vi.fn();
    render(<AnalysisListPage analyses={[analysis]} navigate={nav} />);
    fireEvent.click(screen.getByText("shop.example.org").closest("button")!);
    expect(nav).toHaveBeenCalledWith("/analysis/42");
  });

  it("renders multiple analysis cards", () => {
    const second: Analysis = { ...analysis, id: "99", siteName: "other.example.org", url: "https://other.example.org", findings: [] };
